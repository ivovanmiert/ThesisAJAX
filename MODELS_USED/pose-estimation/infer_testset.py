...
class Pose:
    ...
    def postprocess(self, det, img1, img0, i, j):
        ...
        if len(det):
            ...
            #print(coords)
            print('hallokomthijhier')
            save_path = f'/scratch-shared/ivanmiert/pose-estimation/outputs/{i}/{j}.png'
            print(save_path)
            print(img0)
    ...
    @torch.no_grad()
    def predict(self, image, pred, i, j):
        img = self.preprocess(image)
        #pred = self.det_model(img)[0]  
        coords = self.postprocess(pred, img, image, i, j)
        return coords
...
if __name__ == '__main__':
    ...
    # Define the paths
    detections_folder = '/home/ivanmiert/pose-estimation/detection'
    frames_folder = '/home/ivanmiert/pose-estimation/selected_frames_for_tests'
    output_folder = '/home/ivanmiert/pose-estimation/wat'
    print('hallo')
    ...
    for event_dir in os.listdir(detections_folder):
        ...
        output_csv_path = os.path.join(output_folder, f'{event_dir}_output.csv')
        ...
        csv_file_path = os.path.join(detections_folder, event_dir)
        ...
        #df = pd.read_csv('/home/ivanmiert/pose-estimation/csv/Ball_Occlusion.csv', header=None)
        ...
        output_data = []
        eventname = event_dir.split('.')[0]
        mypath = f"/scratch-shared/ivanmiert/selected_frames_for_tests/{eventname}"
        ...
        for frame_number, group in df:
            ...
            frame_path = os.path.join(frames_folder, f'{eventname}', onlyfiles[frame_number])
            print(frame_path)
            #frame_path = '/home/ivanmiert/pose-estimation/frames/Ball_Occlusion/g5476247_frame_077.jpg'
            ...
            pose_predictions = pose.predict(image, scaled_bboxes, i, j)
        ...
        # output_csv_path = os.path.join(output_folder, f'event_{event_id}_pose_output.csv')
        # output_df.to_csv(output_csv_path, index=False)   
        print(f"Pose predictions saved for event {event_dir} in {output_csv_path}")