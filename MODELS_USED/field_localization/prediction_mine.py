...
#Load the image
def prepare_tensor():
    image = cv2.imread('/scratch-shared/ivanmiert/data/test/00000.jpg')
    image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    tensor = torch.tensor(image).permute(2, 0, 1).unsqueeze(0).float() / 255.0  # Normalize to [0, 1]
    return tensor

@hydra.main(config_path = "/home/ivanmiert/sportlight_folder/soccernet-calibration-sportlight/src/models/hrnet/model_config/", config_name = "hrnet_w48_copy")
def main(cfg):
    print(cfg.stem_width)
    
    tensor = prepare_tensor()
    HRNET = HighResolutionNet(cfg)
    prediction, prediction_internal = HRNET.forward(tensor)
    torch.save(prediction, 'prediction_tensor.pt')

if __name__ == "__main__":
    main()


##with open('/home/ivanmiert/sportlight_folder/soccernet-calibration-sportlight/src/models/hrnet/model_config/hrnet_w48_copy.yaml', 'r') as file:
