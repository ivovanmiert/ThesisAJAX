...
def player_detection(image_path, rescale_img_factor, model_detection, thres_detection):
    ...
    try:
        ...
        # If an error occurs, log it and return empty lists and None for features
...
def light_track(pose_estimator, model_detection, visual_feat_model, layer,
    ...
    if annotation_folder is not None :
        ...
        #print('HIER')
        #print(image_paths, frame_interval, init_frame)
        num_imgs = len(image_paths)
        print(num_imgs)
        #num_imgs = min(n_img_max, len(image_paths)//frame_interval - init_frame)
        total_num_FRAMES = num_imgs

    acc = mm.MOTAccumulator(auto_id=True) #maakt een accumulator aan die geupdate wordt voor ieder frame
    ...
    while img_id < num_imgs-1:
        ...
        if is_keyframe(img_id, keyframe_interval) or flag_mandatory_keyframe :
            ...
            # print(num_dets)
            # print('flag mandatory keyframe:')
            # print(flag_mandatory_keyframe)
            if num_dets <= 0 :
            ...
            if img_id > 0 :   # First frame does not have previous frame
                # print('hierzo:')
                # print(bbox_dets_list_list[img_id - 1])
                # print('hierboven')
                bbox_list_prev_frame = bbox_dets_list_list[img_id - 1].copy()
                track_ids_dict_prev = track_ids_dict_list[img_id - 1].copy()

            # Perform data association
            #In deze for loop worden alle detection id's in de image doorgelopen, en gekeken welke bbox van previous frame het meest overeenkomen met de huidige frame
            # door gebruik van IOU. 
            for det_id in range(num_dets):
                ...
                bbox_dets_list.append(bbox_det_dict)
            # print('nu hierzo::')
            # print(bbox_dets_list)
            #Hier wordt gekeken nadat alle detections zijn doorgelopen of er dubbele detecties zijn, zoja worden deze geremoved. 
            ...
            if img_id > 0 and len(bbox_list_prev_frame) > 0 :
                ...
                # Als de bounding box list van het vorige frame al leeg is omdat alle bounding boxes reassigned zijn, of als use features en useREID module alletwee false zijn    
                if len(bbox_list_prev_frame) == 0 or (not use_features and not use_ReID_module):
                    ...
                    #Eerst worden de image features en image sizes gepak. 
                    ...
                    # print('length bbox list prev frame- daarna itself')
                    # print(len(bbox_list_prev_frame))
                    # print(bbox_list_prev_frame)
                    # print('N_past_to_keep:')
                    # print(N_past_to_keep)

                    for bbox_prev_dict in bbox_list_prev_frame :
                        # print('length bbox prev dict daarna itself:')
                        # print(len(bbox_prev_dict))
                        # print(bbox_prev_dict)
                        ...
                        for past_track_bbox in past_track_bbox_list :
                            ...
                            if use_pose :
                                ...
                                keypoints = []
                            #Als visual features gebruikt worden, worden die hier opgehaald. Als een track al visual features heeft van vorige detecties, wordt dat niet opnieuw gedaan. 
                            if use_visual_feat :
                    ...
                    if not past_track_bbox_list_list[0]:
                        # print('hier ff checken naar remaining_det_id_list of er iets in zit, zodat er wel iets wordt toegevoegd:')
                        # print(remaining_det_id_list)
                        # print(remaining_det_id_list[0])
                        for det_id in remaining_det_id_list :
                    #print('no matching')
                            bbox_dets_list[det_id]["track_id"] = next_id
                            bbox_dets_list[det_id]["method"] = None
                    ...
                    if use_features:
                        # print('eerst length bbox dets list:')
                        # print(len(bbox_dets_list))
                        # print('dan de bbox dets list itself')
                        # print(bbox_dets_list)
                    ...
                    pass
            # print('hij moet hier gewoon komen hoorrrrr')
            # print('kijken of hij wel bij de bbox_dets_list wel is toegevoegd')
            # print(bbox_dets_list)
            # update frame
            bbox_dets_list_list.append(bbox_dets_list)
            track_ids_dict_list.append(track_ids_dict)
    ...
    if visualize :
        ...
        #display_pose = display_pose, flag_trvack = True, flag_method = flag_method)
        #folder ='C:\\Users\\IvovanMiert\\Documents\\VisualStudioCode\\Self-Supervised-Small-Soccer-Player-Detection-Tracking\\scripts\\detection\\results\\test\\testrun'
        img_paths = get_immediate_childfile_paths(image_folder)
        print('HIER IMAGE PATHS')
        print(img_paths)
        if write_video:
            print('write the video!!!')
            ...
            #print("Average FPS: {:.3f}fps".format(total_num_FRAMES / total_time_ALL))
            #print("Average FPS for Detection only : {:.3f}fps".format(total_num_FRAMES / (total_time_DET)))
            #print("Average FPS excluding Detection: {:.3f}fps".format(total_num_FRAMES / (total_time_ALL - total_time_DET)))
            #print("Average FPS for framework only: {:.3f}fps".format(total_num_FRAMES / (total_time_ALL - total_time_DET - total_time_POSE - total_time_FEAT) ))
...
def feature_matching(bbox_dets_list, remaining_det_id_list, bbox_list_prev_frame, past_track_bbox_list_list, track_ids_dict,
    ...
    dist_tab = []
    weight_tab = []
    print('hier de remaining_det_id_list eerst len dan itself:')
    print(len(remaining_det_id_list))
    print(remaining_det_id_list)
    print('hier de length van past_track_bbox list list')
    print(len(past_track_bbox_list_list))
    print(past_track_bbox_list_list[0])
    if (len(past_track_bbox_list_list) != 0):
