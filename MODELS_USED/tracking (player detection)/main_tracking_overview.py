import sys
sys.path.append('/home/ivanmiert/playerDET/scripts/other_utils/lighttrack')
sys.path.append('/home/ivanmiert/playerDET/scripts/other_utils/lighttrack/HPE')
sys.path.append('/home/ivanmiert/playerDET/scripts/other_utils/lighttrack/utils')
sys.path.append('/home/ivanmiert/playerDET/scripts/detection')
...
# Function to load subfolders from a JSON file
...
def main(output_path_csv, image_folder, subfolders_to_process):
    parser = ArgumentParser()
    parser.add_argument('--data_name', type=str, default='testrun')
    ...
    parser.add_argument('--checkpoint', type=str, default='/home/ivanmiert/playerDET/checkpoints_runs/player_det_resnet18_student.pth')
    parser.add_argument('--detection_score_thres', type=float, default=0.95)
    ...
    parser.add_argument('--w_spacial', type=float, default=0.6)
    parser.add_argument('--w_visual', type=float, default=0.4)
    ...
    parser.set_defaults(use_ReID_module=True)
    parser.add_argument('--max_vis_reID', type=int, default=4)
    parser.add_argument('--max_vis_feat', type=int, default=4)
    parser.add_argument('--N_past_to_keep_reID', type=int, default=30)
    parser.add_argument('--N_past_to_keep', type=int, default=30)
    parser.add_argument('--N_frame_lost_keep', type=int, default=30)
    ...
    parser.add_argument('--output_path', type=str, default= output_path_csv)#'/scratch-shared/ivanmiert/final_events_folder/detection3000')

    hparams = parser.parse_args()

    print(f"use visual feat: {hparams.use_visual_feat}")
    ...
    # # Load the JSON file with the list of subfolders to process
    ...
    if not hparams.use_GT_position:
        print('GT_POSITION USE IS FALSE')
        if hparams.current_model_detection is None:
            print('Current_model_detection is none')
            from train_tracker import get_model_detection
            print(hparams.weight_loss, hparams.backbone, hparams.detection_score_thres, hparams.use_soft_nms, hparams.anchor_sizes, hparams.use_context, hparams.nms_thres, hparams.use_track_branch_model)
            ...
            print(model_detection.load_state_dict(torch.load(hparams.checkpoint)))
    ...
    if hparams.use_visual_feat:
        if hparams.visual_feat_model_name == 'faster-rcnn':
            print('visual feat model is faster-rcnn')
            if hparams.current_model_detection is None:
                from train_tracker import get_model_detection
    ...
    if hparams.data_name == 'testrun':
        #base_image_folder = '/scratch-shared/ivanmiert/frames_map/'
        base_image_folder = image_folder
        #base_image_folder = '/scratch-shared/ivanmiert/final_events_folder/frames_10000/'
    ...
    if hparams.data_name == 'SPD':
        ...
        rescale_bbox = [0., 0.]
    i = 0 
    #print(f"subfolders to use: {subfolders_to_use}")
    for s in natsorted(os.listdir(base_image_folder), alg=ns.PATH | ns.IGNORECASE):
        #print(f"s: {s}")
        if s not in subfolders_to_process:
            print(f"skip {s}")
            continue
        #print(f"zoveel: {i}")
        i+= 1
        try:
            event_id = int(s.split('_')[1])
        except (IndexError, ValueError):
            print(f"Invalid folder name format: {s}")
            continue
